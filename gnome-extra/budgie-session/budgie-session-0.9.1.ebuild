...
src_configure() {
	local emesonargs=(
		...
		$(meson_use elogind)
		-Dsession_selector=false # will otherwise look for gnome-custom-session and crash
		$(meson_use systemd)
...
src_install() {
	...
	doexe "${FILESDIR}/Budgie-Desktop"

	newmenu "${FILESDIR}/defaults.list-r6" budgie-mimeapps.list

	exeinto /etc/X11/xinit/xinitrc.d/
	newexe "${FILESDIR}/15-xdg-data-budgie-r1" 15-xdg-data-budgie

	# This should be done here as discussed in bug #270852
	newexe "${FILESDIR}/10-user-dirs-update-budgie-r1" 10-user-dirs-update-budgie

	# Set XCURSOR_THEME from current dconf setting instead of installing
	# default cursor symlink globally and affecting other DEs (bug #543488)
	# https://bugzilla.gnome.org/show_bug.cgi?id=711703
	newexe "${FILESDIR}/90-xcursor-theme-budgie" 90-xcursor-theme-budgie
}
...
pkg_postinst() {
	...
	if ! use systemd && ! use elogind; then
		...
		ewarn "budgie-session won't be able to correctly track and manage your session."
	fi
}
